...
{{#include ac.md:12:22}}
    <tr>
        <td><b>SC-32</b></td>
        <td><b>系统分区，System Partitioning</b></td>
        <td></td>
        <td></td>
        <td></td>
    </tr>
...
<u>控制增强</u>：（1）<u>OT 讨论</u>：组织应考虑为特权功能，如影响安全和安全性的功能，划分独立的物理域。
...
{{#include ac.md:12:22}}
    <tr>
        <td><b>SC-39</b></td>
        <td><b>进程隔离，Process Isolation</b></td>
        <td><b>选择</b></td>
        <td><b>选择</b></td>
        <td><b>选择</b></td>
    </tr>
...
<u>将 SC-41 添加到 LOW、MOD 和 HIGH 基线的理由</u>：OT 功能通常是提前定义的，并且不会经常更改。
...
<u>OT 讨论</u>：组织应协调 OT 上的时间同步，以便进行准确的故障排除和取证。

<u>控制增强</u>：（1）<u>OT 讨论</u>：当数据跨组织边界关联时，可以选择与权威时间源同步作为控制。 OT 采用合适的时间戳机制（例如 GPS、IEEE 1588）。

<u>将 SC-45 添加到 LOW、MOD 和 HIGH 基线的理由</u>：组织可能会发现相对的系统时间有利于许多 OT 系统，以确保基本功能安全并可靠的交付。时间同步还可以确保来自不同系统的审核日志保持一致，从而使组织在聚合日志时，能够准确查看多个系统的事件，从而使根本原因分析更加高效。
...
<u>OT 讨论</u>：组织应考虑哪些系统需要备用通信路径，以确保通信中断不会导致不可接受的视图或控制丢失或安全事件。

<u>将 SC-47 添加到高基线的理由</u>：为了在事件期间保持操作的连续性，组织应考虑建立用于命令和控制目的的备用通信路径，以便在可用性或完整性丧失可能导致严重或灾难性的不利影响（包括对安全和关键服务交付的影响）时，继续操作高影响系统并采取适当的行动。
...
<u>OT 讨论</u>：一些 OT 系统通过实现物理按键开关，或写保护开关来支持写保护。组织应定义这些启用写保护的系统，并开发如何使系统脱离写保护模式的流程。
